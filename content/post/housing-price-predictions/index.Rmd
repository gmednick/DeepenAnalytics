...
sale_price_dist <- ggplot(ames_df, aes(x = sale_price)) + 
  geom_histogram(bins = 50, fill = "#c9ecb2") +
...
p <- ames_df %>% select(neighborhood, year_sold, sale_price) %>% 
  ...
  arrange(year_sold) %>% 
  ggplot(aes(year_sold, sale_price, fill = sale_price)) +
  geom_boxplot() +
  theme(legend.position = 'none') +
...
Feature engineering with the `recipes` package, provides an arsenal of data transformations that may be necessary and beneficial to the predictive power of our trained machine learning model. 
