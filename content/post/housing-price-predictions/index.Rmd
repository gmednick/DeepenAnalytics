...
Since sale price is the feature of interest, let's make a closer inspection by plotting it. Even before we look at a histogram of sale price, we can imagine that the distribution might have a bell-shape with a skewed tale on the right owing to a few uber-expensive mansions. In that case, we will want to apply a log transformation to the sale price before modeling. We will take this into consideration when we get to data engineering.
...
How has sale price changed with time?
```{r}
